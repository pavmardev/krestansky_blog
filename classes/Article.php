...
class Article {
    ...
    public function find_article($article_id) {
        ...
        $sql = "SELECT meno, priezvisko, clanky.datum AS clanky_datum, nazov, text_clanku, kategoria FROM pouzivatelia INNER JOIN clanky ON pouzivatelia.id = clanky.pouzivatelia_id INNER JOIN kategorie ON clanky.kategorie_id = kategorie_id WHERE clanky.id = :id";
        $stmt = $this->db->prepare($sql);
        ...
        $us_name = $result['meno'];
        $us_sur = $result['priezvisko'];
        $date = $result['clanky_datum'];
        $art_name = $result['nazov'];
        $art_text = $result['text_clanku'];
        $category = $result['kategoria'];
        return array($us_name, $us_sur, $date, $art_name, $art_text, $category);
    }
